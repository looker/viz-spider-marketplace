...
function RadarChart(id, data, options, moreData, colorSeries) {
	var cfg = {
   ...
   axisColor: "#CDCDCD"
	...
	//Append the lines
	axis.append("line")
	  .attr("x1", function(d, i){ return cfg.negatives ? rScale(maxValue*-1) * Math.cos(angleSlice*i - Math.PI/2) : 0; })
		.attr("y1", function(d, i){ return cfg.negatives ? rScale(maxValue*-1) * Math.sin(angleSlice*i - Math.PI/2) : 0; })
  ...
  if (cfg.roundStrokes) {
	...
	var radarLine = d3.svg.line.radial()
		...
		.radius(function(d) { return rScale(!cfg.negatives ? d.value < 0 ? 0 : d.value : d.value); })
  ...
  //console.log(moreData);
	...
	blobWrapper.selectAll(".radarCircle")
		...
		.attr("cx", function(d,i){ return rScale(!cfg.negatives ? d.value < 0 ? 0 : d.value : d.value) * Math.cos(angleSlice*i - Math.PI/2); })
		.attr("cy", function(d,i){ return rScale(!cfg.negatives ? d.value < 0 ? 0 : d.value : d.value) * Math.sin(angleSlice*i - Math.PI/2); })
...
const visObject = {
  ...
  **/
    ...
    marginY = element.clientHeight*.1;
    ...
    svg.append("g")
    	.call(RadarChart("#vis", formattedData, radarChartOptions2, moreData, series_colors2));
