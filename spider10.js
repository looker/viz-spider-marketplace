...
function RadarChart(id, data, options, moreData, colorSeries, originalData, axes) {
  ...
  svg.append("g")
    ...
    .attr("transform", function(d) { return `translate(${legx - data.length*leg_pad/2},${legy})`}) //`translate(${legx - data.length*leg_pad/2},${legy})`
    .style("font-family", "Open Sans");
...
const visObject = {
  ...
  **/
    ...
    //console.log(marginY);
    var margin = {top: marginY, right: marginX, bottom: config.legend_side === 'center' ? marginY*1.15 : marginY, left: marginX},
    ...
    const visOptions = {};
    Object.assign(visOptions, baseOptions);
	...
	series_colors = [];
	series.forEach(function(s) {
      series_colors.push(config[`${s}_color`]);
  	});

  	var color = d3.scale.ordinal().range(series_colors);
    ...
    svg.append("g")
    	.call(RadarChart("#vis", formattedData, radarChartOptions1, moreData, series_colors, originalData, axes));
